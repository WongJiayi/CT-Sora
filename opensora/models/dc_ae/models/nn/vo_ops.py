...
def pixel_shuffle_3d(x, upscale_factor):
    ...
    B, C, T, H, W = x.shape
    r = upscale_factor
    assert C % (r * r * r) == 0, "Number of channels must be a cubic multiple of the upscale factor"
...
def pixel_unshuffle_3d(x, downsample_factor):
    ...
    r = downsample_factor
    assert T % r == 0, f"Temporal dimension must be divisible by downsample factor, got shape {x.shape}"
    assert H % r == 0, f"Height dimension must be divisible by downsample factor, got shape {x.shape}"
    assert W % r == 0, f"Width dimension must be divisible by downsample factor, got shape {x.shape}"
    T_new = T // r
    H_new = H // r
    W_new = W // r
    C_new = C * (r * r * r)
